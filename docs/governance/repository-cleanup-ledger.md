...
| Local and remote branches | Approved merged-PR branch cleanup group: `docs/roadmap-delivery-governance`, `feat/phase-1-ci-docs-final`, `feat/phase-1-pr1-scope-guardrails`, `feat/phase-1-pr2-config-readiness`, `feat/phase-1-pr3-ci-smoke-polish`. | Engram `sdd/strict-tdd-and-deep-cleanup/apply-progress`; verified absent from local branches, remote-tracking refs, and `git ls-remote --heads origin`. | destructive-action-required | Preserve branch names, merged PR context, and before/action/after evidence in Engram. | Explicit user approval for this five-branch allow-list only. | Deleted approved local refs with `git branch -d`; deleted approved remote refs with `git push origin --delete`. | Absence verified locally, in remote-tracking refs, and in origin heads; `pnpm public-safe` passed. | Completed |
| Local branches | Any local branch outside the approved cleanup group. | Read-only branch inventory; unapproved refs were not targeted during approved cleanup. | needs-owner-review | Preserve branch names and reachability evidence before deletion. | Required before any future local branch deletion. | No action for unapproved branches. | Must verify branch list before any future deletion. | Pending |
| Remote branches | Any remote branch outside the approved cleanup group. | Read-only remote inventory; unapproved refs were not targeted during approved cleanup. | destructive-action-required | Preserve merged/closed PR mapping and latest commit before deleting remote refs. | Required before any future remote branch deletion. | No action for unapproved remote branches. | Must verify remote-tracking refs and `git ls-remote --heads origin` before any future deletion. | Pending |
...
- Dropping stashes.
- Deleting any local or remote branch outside the approved completed five-branch cleanup group.
